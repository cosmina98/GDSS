...
def graph_load_batch(min_num_nodes=30, max_num_nodes=1000, name='ENZYMES', node_attributes=True, graph_labels=True):
    """
   ...
   if dataset == 'community_small':
    ...
    try:
        graphs = load_custom_graph_list(
            r'dataset\{}\{}\{}.p'.format(dataset[:dataset[:dataset.rindex('_', 0, dataset.rfind('_'))].rfind('_')]\
        ...
        for G  in graphs:
            ...
            nx.set_node_attributes(G, labels, name='feature')

    except: raise NotImplementedError(f'Dataset {dataset} not supported.')
